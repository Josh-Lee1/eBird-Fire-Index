...
X145_Feeding_habitat_Urban_landscapes_9) %>% 
  mutate(movetotal = X193_National_movement_local_dispersal_13 +
           ...
           X197_National_movement_Irruptive_13) %>% 
  mutate(localonly = movetotal==X193_National_movement_local_dispersal_13) %>% 
  mutate(localdisperse = X193_National_movement_local_dispersal_13 == 1)
```
...
modmobility<-lm(Estimate ~ localdisperse, data = valuesfin,weights = 1/`Std. Error`)
modlocalonly<-lm(Estimate ~ localonly, data = valuesfin,weights = 1/`Std. Error`)

summary(modfood)
summary(modhabitat)
summary(modsize)
summary(modmobility)
summary(modlocalonly)
...
localonly<- broom::glance(modlocalonly)

outputs<- bind_rows(habitat, food, size, localonly)
outputs$test <- c("Habitat Specialism", "Food Specialism", "Body Size", "Obligate Sedentism")
...
food <- ggplot(modfood, aes(foodspec, Estimate*-1)) + geom_point() + theme_bw() + xlab("Number of Feeding Guilds") + ylab("Modelled Response to Fire") + ggtitle("A")
ggsave("Output/figures/foodmodel.PNG")
size <- ggplot(modsize, aes(X99_Body_mass_average_8, Estimate*-1)) + geom_point()+ scale_x_log10()+ theme_bw() + xlab("Body Mass (grams)") + ylab("Modelled Response to Fire") + ggtitle("D")
ggsave("Output/figures/sizemodel.PNG")
hab <- ggplot(modhabitat, aes(habitatspec, Estimate*-1)) + geom_point()+ theme_bw() + xlab("Number of Habitats") + ylab("Modelled Response to Fire") + ggtitle("B")
ggsave("Output/figures/habitatmodel.PNG")
#mob <- ggplot(modmobility, aes(localdisperse, Estimate*-1)) + geom_point()+ theme_bw() + xlab("Locally #Dispersing") + ylab("Modelled Response to Fire")
#ggsave("Output/figures/mobilitymodel.PNG")
loc <- ggplot(modlocalonly, aes(localonly, Estimate*-1)) + geom_point() + theme_bw() + xlab("Sedentary Only") + ylab("Modelled Response to Fire") + ggtitle("C")
ggsave("Output/figures/Osed.PNG")
```
```{r}
library(patchwork)
patch<- (food | hab)/(loc | size)
print(patch)
```
